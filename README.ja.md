...
- **ツール詳細の強化**: Claude の stream-json に含まれる `Grep` の検索パターン/対象パス、`ScheduleWakeup` の待機時間/理由、`WebFetch` の URL とプロンプト要約、`WebSearch` のクエリと include/exclude ドメイン件数、`ToolSearch` のクエリと `max_results`、`Monitor` の説明/タイムアウト、`TaskStop` の task id、Context7 MCP ツールの library/query を表示
