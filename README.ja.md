...
- **入れ子起動の深さ**: サブエージェントがさらにサブエージェントを起動した場合、`task_started` の行に `depth:<n>` を添える。`result.subagent_stats.max_depth` は最終集計しか持たないため、これが無いと「どのタスクが深いのか」を追えない。入れ子は実行時間とトークン消費が指数的に膨らむ要因そのもの
