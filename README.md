...
- **Observed stream-json edge cases**: Suppresses high-frequency `background_tasks_changed` snapshots already represented by task events, shows optional Agent `model` / `isolation` launch settings, marks `isImage:true` tool results as `image`, and counts every `structuredPatch[].lines` entry beginning with `+` or `-` (including added/removed content that itself begins with `++` / `--`)
