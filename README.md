...
- **Tool error summary**: When a `tool_result` is `is_error:true`, the live monitor appends a short, single-line summary (truncated to 120 characters, with `<tool_use_error>` wrappers stripped) so the cause of a failed tool call is visible without opening the jsonl
