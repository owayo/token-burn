...
- **Richer tool details**: Shows `Read` offset/limit, `Edit` replace-all state, `Bash` timeout/background state, `Agent` background state, `Grep`/`Glob` output mode, ignore-case, glob, head/context limits, delay/reason for `ScheduleWakeup`, URL/prompt summary for `WebFetch`, query/domain filters for `WebSearch`, query/`max_results` for `ToolSearch`, monitor description/timeout for `Monitor`, stopped task ID for `TaskStop`, task ID/block/timeout for `TaskOutput`, `TaskCreate` subject/description, `TaskUpdate` task ID/status/owner, `SendMessage` summaries, `AskUserQuestion` prompts/options, Tavily/Codex MCP sandbox/approval details, and library/query details for Context7 MCP tools
