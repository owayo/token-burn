...
- **Session total including subagents**: `result.usage` only accumulates the main loop, so whenever the `modelUsage` totals exceed it, a `📊 total in:<n> out:<n> (incl. subagents)` line is added. Real logs show cache_read diverging from `2,110,689` to `220,321,325` — over 100x — so the headline `in/out` alone understates real consumption by orders of magnitude. Sessions without subagents match exactly and the line is omitted
...
| `rate_limit_threshold` | Auto-stop when the five-hour / seven-day window utilization reaches this percentage (`1-100`); the monthly overage window never triggers it | `95` (default) |
