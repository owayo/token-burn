...
* [Audit configuration changes](https://code.claude.com/docs/en/hooks-guide.md#audit-configuration-changes)
...
The matcher filters by configuration type: `user_settings`, `project_settings`, `local_settings`, `policy_settings`, or `skills`. To block a change from taking effect, exit with code 2 or return `{"decision": "block"}`. See the [ConfigChange reference](https://code.claude.com/en/hooks#configchange) for the full input schema.
...
* `"type": "http"`: POST event data to a URL. See [HTTP hooks](https://code.claude.com/docs/en/hooks-guide.md#http-hooks).
* `"type": "prompt"`: single-turn LLM evaluation. See [Prompt-based hooks](https://code.claude.com/docs/en/hooks-guide.md#prompt-based-hooks).
* `"type": "agent"`: multi-turn verification with tool access. See [Agent-based hooks](https://code.claude.com/docs/en/hooks-guide.md#agent-based-hooks).
...
Your script can parse that JSON and act on any of those fields. `UserPromptSubmit` hooks get the `prompt` text instead, `SessionStart` hooks get the `source` (startup, resume, clear, compact), and so on. See [Common input fields](https://code.claude.com/en/hooks#common-input-fields) in the reference for shared fields, and each event's section for event-specific schemas.
...
The endpoint should return a JSON response body using the same [output format](https://code.claude.com/en/hooks#json-output) as command hooks. To block a tool call, return a 2xx response with the appropriate `hookSpecificOutput` fields. HTTP status codes alone cannot block actions.
...
For full configuration options and response handling, see [HTTP hooks](https://code.claude.com/en/hooks#http-hook-fields) in the reference.
