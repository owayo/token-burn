...
`isImage` は実データで `false` が常設されるため、`true` の場合だけ `image` を表示します。`structuredPatch[].lines` は hunk 内の行だけを保持しファイルヘッダーを含まないため、`+` / `-` で始まる全行を加除として数えます。これにより、内容自体が `++` / `--` で始まる行が diff 上で `+++` / `---` になっても過少計上しません。
